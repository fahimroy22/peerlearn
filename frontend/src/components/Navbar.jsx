...
function Navbar() {
  ...
  const handleNotificationClick = async (n) => {
    try {
      if (!n.isRead) {
        ...
        setNotifications((p) =>
          p.map((x) => (x._id === n._id ? { ...x, isRead: true } : x))
        );
  ...
  const notifIconClass = useCallback((type) => {
    const map = {
      session_message: "nv-icon-indigo",
      request_message: "nv-icon-blue",
      learn_request: "nv-icon-orange",
      request_accepted: "nv-icon-green",
      session_created: "nv-icon-purple",
      exchange_request: "nv-icon-pink",
      exchange_request_accepted: "nv-icon-yellow",
      exchange_message: "nv-icon-cyan",
      support_ticket_created: "nv-icon-blue",
      support_reply: "nv-icon-indigo",
  ...
  const groupedNotifs = useMemo(() => {
    ...
    return [
      { title: "Today", items: t },
      { title: "Yesterday", items: y },
      { title: "Earlier", items: e },
      ...
      <div className="nv-shell">
        <div className="nv-inner">
          <Link to="/" className="nv-brand" aria-label="PeerLearn Home">
            <span className="nv-brand-mark">
              <span className="nv-brand-icon">P</span>
            </span>
            <span className="nv-brand-copy">
          ...
          <div className={`nv-links ${mobileOpen ? "is-open" : ""}`}>
            <div className="nv-links-left">
              {navLinks.map(({ to, label, badge }) => (
                <Link key={to} to={to} className={`nv-link ${isActive(to) ? "is-active" : ""}`}>
                  <span className="nv-link-label">{label}</span>
            ...
            <div className="nv-links-right">
              {user ? (
                <>
                  <div className={`nv-notif ${notificationOpen ? "is-open" : ""}`} ref={notificationMenuRef}>
                    <button
                      type="button"
                      className={`nv-bell-btn ${bellPulse ? "has-pulse" : ""} ${
                    ...
                    >
                      ...
                      <span className="nv-mobile-control-label">Notifications</span>

                      {notificationUnreadCount > 0 && (
                        <span className="nv-bell-badge">
                    ...
                    <div className="nv-notif-panel">
                      <div className="nv-notif-head">
                        <div>
                          ...
                          <div className="nv-notif-subtitle">
                            Activity across chats, requests and sessions
                          </div>
                        </div>
                  ...
                  <div className={`nv-profile ${profileOpen ? "is-open" : ""}`} ref={profileMenuRef}>
                    <button
                      type="button"
                    ...
                    <div className="nv-profile-panel">
                      ...
                      {isAdmin ? (
                        ...
                        <>
                          <Link to="/dashboard" className="nv-pm-item" style={{ "--di": 0 }}>
                            Dashboard
                          </Link>
                          <Link to="/edit-profile" className="nv-pm-item" style={{ "--di": 1 }}>
                            Edit Profile
                          </Link>
                          <Link to="/support/my" className="nv-pm-item" style={{ "--di": 2 }}>
                            My Support Tickets
                          </Link>
                        </>
                      )}
                ...
                <>
                  <Link to="/login" className={`nv-link ${isActive("/login") ? "is-active" : ""}`}>
                    Login
                  </Link>
